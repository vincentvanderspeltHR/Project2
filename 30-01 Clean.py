...
class Game:
    def __init__(self, firstplayer, Player1, Player2):
    ...
    def nextplayer_ingame(self):
        ...
        if valid_turn == len(Game1.currentplayer.boatlist):
            ...
            if Game1.setup_counter == 9:
                text_to_screen(screen, "Er zijn boten die elkaar overlappen!", red, -display_height * 0.48)
    ...
    def nextplayer_setup(self):
        if self.currentplayer.currentboat.confirm():
            ...
            text_to_screen(screen, "Er staan boten buiten het veld of er overlappen boten!", red, -display_height * 0.48)
...
class Player:
    ...
    def show_stats(self, scherm):
        text_to_screen(scherm, "HP: " + str(self.currentboat.currenthp) + "/" + str(self.currentboat.hp), black,
                       -display_height * 0.45,
                       "small", -display_width * 0.45)
        text_to_screen(scherm, "Stappen: " + str(self.currentboat.movement) + "/" + str(self.currentboat.steps), black,
                       -display_height * 0.425, "small", -display_width * 0.414)
        text_to_screen(scherm, "Aanval: " + str(self.currentboat.attack_amount) + "/" +
                       str(self.currentboat.original_attack_amount), black, -display_height * 0.4, "small",
                       -display_width * 0.422)

    def show_target_stats(self, scherm, vijand):
        display_difference = 0.025
        enemy_boat = 3

        if self.targeted_boat == vijand.boatlist[0]:
            text_to_screen(scherm, "HP: " + str(self.targeted_boat.currenthp) + "/" + str(self.targeted_boat.hp), red,
    ...
    @staticmethod
    def show_enemy_stats(scherm, vijand):
        display_difference = 0.025
        enemy_boat = 0

        for boat in vijand.boatlist:
            enemy_boat += 1
            text_to_screen(scherm, "Schip " + str(enemy_boat), black,
                           -(display_height * 0.5) + (display_difference * display_height * enemy_boat * 3), "small",
                           +display_width * 0.40)
            text_to_screen(scherm, "HP: " + str(boat.currenthp) + "/" + str(boat.hp), black,
                           -(display_height * 0.475) + (display_difference * display_height * enemy_boat * 3), "small",
                           +display_width * 0.40)

    def selectedboat(self, scherm):
        if self.currentboat.new_stance == "attacking":
            pygame.draw.ellipse(scherm, (255, 255, 255), (self.currentboat.new_x, self.currentboat.new_y,
                                                          self.currentboat.gamegrid.gridx -
                                                          (self.currentboat.gamegrid.gridx / 4),
                                                          (self.currentboat.gamegrid.gridy * self.currentboat.length -
                                                           (self.currentboat.gamegrid.gridx / 4))), 4)
        else:
            pygame.draw.ellipse(scherm, (255, 255, 255), (self.currentboat.new_x, self.currentboat.new_y,
                                                          (self.currentboat.gamegrid.gridy * self.currentboat.length -
                                                           (self.currentboat.gamegrid.gridx / 4)),
    ...
    def draw_targetedboat(self, scherm):
        if self.targeted_boat.new_stance == "attacking":
            pygame.draw.ellipse(scherm, (255, 0, 0), (self.targeted_boat.new_x, self.targeted_boat.new_y,
                                                      self.targeted_boat.gamegrid.gridx -
                                                      (self.targeted_boat.gamegrid.gridx / 4),
                                                      (self.targeted_boat.gamegrid.gridy * self.targeted_boat.length -
                                                       (self.targeted_boat.gamegrid.gridx / 4))), 4)
        else:
            pygame.draw.ellipse(scherm, (255, 0, 0), (self.targeted_boat.new_x, self.targeted_boat.new_y,
                                                      (self.targeted_boat.gamegrid.gridy * self.targeted_boat.length -
                                                       (self.targeted_boat.gamegrid.gridx / 4)),
    ...
    def attack(self, boat, vijand):
        if boat.emp_buff > 0:
            boat.emp_buff -= 1
            game_error("Aanval geblokkeerd door de EMP buff!", screen)
        else:
            ...
            if Game1.currentplayer.currentboat.damage_buff > 0:
                ...
                game_error("Damage buff gebruikt!", screen)
            if boat.currenthp <= 0:
                game_error("Schip van " + str(vijand.name) + " gezonken!", screen)
                vijand.boatlist.remove(boat)
                if not vijand.boatlist:
                    Game1.currentplayer.score += 1
                else:
                    vijand.currentboat = vijand.boatlist[0]
        self.targeted_boat = 0
    ...
    def draw_cards(self, scherm):
        drawn_cards = 0
        card_amount = len(self.cards_in_hand)
        while drawn_cards != card_amount:
            image = pygame.transform.scale(card.image, (int((display_width * 0.8) / 6), int(display_height * 0.15)))
            scherm.blit(image,
                        [0 + int((display_width * 0.8) / 6) * drawn_cards, GameGrid.gridstarty * 1.7 + GameGrid.y])
            drawn_cards += 1


    @staticmethod
    def draw_from_deck(deck, draw_amount):
        while draw_amount > 0:
            draw_random = random.randint(0, len(deck))
            Game1.currentplayer.cards_in_hand.append(deck[draw_random])
            Game1.normal_deck.remove([draw_random])
            draw_amount -= 1
...
class Grid:
    ...
    def draw(self, scherm):
        pygame.draw.rect(scherm, blue, (self.gridstartx, self.gridstarty, self.x, self.y), 0)
        gridlines = 0
        while not gridlines > 20:
            pygame.draw.line(scherm, (0, 0, 0), (
            ((self.resolution_x - self.x) / 2), (((self.resolution_y - self.y) / 2) + self.gridy * gridlines)), (
                             (((self.resolution_x - self.x) / 2) + self.x),
                             (((self.resolution_y - self.y) / 2) + self.gridy * gridlines)), 2)
            gridlines += 1
        gridlines = 0
        while not gridlines > 20:
            pygame.draw.line(scherm, (0, 0, 0), (
            ((self.resolution_x - self.x) / 2) + self.gridx * gridlines, ((self.resolution_y - self.y) / 2)), (
                             ((self.resolution_x - self.x) / 2) + self.gridx * gridlines,
                             ((self.resolution_y - self.y) / 2) + self.y), 2)
            gridlines += 1
        pygame.draw.rect(scherm, black, (
        self.gridstartx - (display_width * 0.135 - 5) - 8, self.gridstarty, display_width * 0.135, self.y / 3), 8)
        pygame.draw.rect(scherm, black, (
        self.gridstartx - (display_width * 0.135 - 5) - 8, self.y + self.gridstarty - (self.y / 3),
        display_width * 0.135, self.y / 3), 8)
        pygame.draw.rect(scherm, black, (
            self.gridstartx + self.x + 4, self.gridstarty + (self.y / 10) * 4, self.gridstartx, (self.y / 10) * 2), 8)
        trapcards = 0
        while not trapcards > 6:
            pygame.draw.rect(scherm, black, (
            self.gridstartx + (self.x / 7 * trapcards), self.gridstarty - self.gridstarty * 0.75 - 4, self.x / 7,
            self.gridstarty * 0.75), 8)
            trapcards += 1
        pygame.draw.rect(scherm, black, (
        self.gridstartx, self.gridstarty - self.gridstarty * 0.75 - 4, self.x, self.gridstarty * 0.75), 8)
        trapcards = 0
        while not trapcards > 6:
            pygame.draw.rect(scherm, black, (
            self.gridstartx + (self.x / 7 * trapcards), self.gridstarty + self.y + 4, self.x / 7,
            self.gridstarty * 0.75), 8)
            trapcards += 1
        pygame.draw.rect(scherm, black, (self.gridstartx, self.gridstarty + self.y + 4, self.x, self.gridstarty * 0.75),
                         8)
        perkcards = 0
        while not perkcards > 3:
            pygame.draw.rect(scherm, black, (
            self.gridstartx + self.x + 4, self.gridstarty + (self.y / 10) * perkcards, self.x / 4, self.y / 10), 8)
            perkcards += 1
        perkcards = 1
        while not perkcards > 4:
            pygame.draw.rect(scherm, black, (
            self.gridstartx + self.x + 4, self.gridstarty + self.y - (self.y / 10) * perkcards, self.x / 4,
            self.y / 10), 8)
            perkcards += 1
        pygame.draw.rect(scherm, black, (self.gridstartx - 4, self.gridstarty - 4, self.x + 8, self.y + 8), 8)


class Boat:
    def __init__(self, x, y, length, steps, gamegrid, HP, currentHP, attacking_range_x, attacking_range_y,
                 defending_range_y):
        self.x = x
        self.y = y
        self.new_x = x
        ...
        self.new_position = (self.new_x, self.new_y, self.gamegrid.gridx - (self.gamegrid.gridx / 4),
                             (self.gamegrid.gridy * self.length - (self.gamegrid.gridx / 4)))
    ...
    def draw(self, scherm):
        ...
        if self.original_stance == "attacking":
            pygame.draw.ellipse(scherm, color, (self.x, self.y, self.attackingboat_width, self.attackingboat_height), 0)
        elif self.original_stance == "defending":
            pygame.draw.ellipse(scherm, color, (self.x, self.y, self.defendingboat_width, self.defendingboat_height), 0)

    def draw_new(self, scherm):
        if Game1.setup_counter < 9:
            pygame.draw.ellipse(scherm, light_red,
                                (self.new_x, self.new_y, self.attackingboat_width, self.attackingboat_height), 0)
        elif not self.original_stance == self.new_stance and self.new_stance == "defending":
            pygame.draw.ellipse(scherm, light_red,
                                (self.new_x, self.new_y, self.defendingboat_width, self.defendingboat_height), 0)
            ...
            if not self.movement == self.steps:
                ...
                pygame.draw.ellipse(scherm, light_red,
                                    (self.x, self.y, self.attackingboat_width, self.defendingboat_height), 0)
            ...
            if self.new_stance == "defending" and self.movement == self.steps - 1:
                pygame.draw.ellipse(scherm, black,
                                    (self.x, self.y, self.defendingboat_width, self.defendingboat_height), 0)
            else:
                pygame.draw.ellipse(scherm, light_red,
                                    (self.new_x, self.new_y, self.defendingboat_width, self.defendingboat_height), 0)
        ...
        if self.new_stance == "attacking":
            ...
            while not draw_grids == self.horizontal_attackingrange:
                ...
                if startpunt_x - self.gamegrid.gridx * draw_grids < self.gamegrid.gridstartx:
                    ...
                    pygame.draw.rect(scherm, yellow, (
                    startpunt_x - self.gamegrid.gridx * draw_grids, startpunt_y, self.gamegrid.gridx,
            ...
            while not draw_grids == self.horizontal_attackingrange:
                draw_grids += 1
                if startpunt_y + self.gamegrid.gridy * (
                    draw_grids + self.length) > self.gamegrid.gridstarty + self.gamegrid.y:
                    break
                else:
                    pygame.draw.rect(scherm, yellow, (
                        startpunt_x, startpunt_y + self.gamegrid.gridy * (draw_grids + (self.length - 1)),
                        self.gamegrid.gridx,
                        self.gamegrid.gridy))
                if draw_grids == self.vertical_attackingrange:
                    break
        ...
        if self.new_stance == "defending":
            ...
            while not draw_grids == self.vertical_defendingrange:
                ...
                if startpunt_y - self.gamegrid.gridy * draw_grids < self.gamegrid.gridstarty:
                    ...
                    pygame.draw.rect(scherm, yellow, (
                        startpunt_x, startpunt_y - self.gamegrid.gridy * draw_grids, self.gamegrid.gridx * self.length,
                        self.gamegrid.gridy))
        ...
        if self.movement > 0:
            if self.new_stance == "defending":
                if self.switch_x > display_width / 2:
                    self.new_x += self.gamegrid.gridx * (self.length - 1)
                self.new_stance = "attacking"
                if Game1.currentplayer == P2:
                    self.new_y = self.new_y - (self.gamegrid.gridy * 0.6) - self.gamegrid.gridy * (self.length - 1) + (
                    self.gamegrid.gridy * 0.6)
            ...
            game_error("Niet genoeg stappen over om van positie te wisselen!", screen)
    ...
    def move(self, direction):
        if self.new_stance == "attacking":
                ...
                if self.original_stance == "defending" and self.switch_x > display_width / 2:
                    if (self.new_x - self.gamegrid.gridx) < self.switch_x:
                        if self.new_x - self.gamegrid.gridx > self.gamegrid.gridstartx:
                            ...
                            game_error("Niet genoeg stappen over om te bewegen!", screen)
                    else:
                        self.movement += 1
                        self.new_x -= self.gamegrid.gridx
                else:
            ...
            game_error("Verdedigende schepen kunnen niet bewegen!", screen)
                            ...
                            if boat.new_stance == "attacking":
                                if boat.new_x - (self.gamegrid.gridx / 6) < self.new_x < boat.new_x - (
                                    ...
                                    if boat.new_y - (
                                        ...
                                        if Game1.setup_counter == 9:
                                            game_error("Er zijn overlappende schepen!", screen)
                                        return False
    ...
    def attack_check(self, vijand):
        if self.confirm():
                ...
                while tiles >= 0:
                    if self.new_stance == "attacking":
                        if boat.original_stance == "attacking":
                            if self.new_x - (self.gamegrid.gridx / 6) + self.gamegrid.gridx * (
                                self.horizontal_attackingrange + 1) > boat.x > self.new_x - (
                                self.gamegrid.gridx / 6) - self.gamegrid.gridx * self.horizontal_attackingrange:
                                if self.new_y - (
                                    self.gamegrid.gridy / 6) + self.gamegrid.gridy * self.length > boat.y + self.gamegrid.gridy * tiles > self.new_y - (
                                    self.gamegrid.gridy / 6):
                                    attackable = True
                            if self.new_x - (self.gamegrid.gridx / 6) + self.gamegrid.gridx > boat.x > self.new_x - (
                                self.gamegrid.gridx / 6):
                                if self.new_y - (self.gamegrid.gridy / 6) + self.gamegrid.gridy * (
                                    self.length + self.vertical_attackingrange) > boat.y + self.gamegrid.gridy * tiles > self.new_y - (
                                    self.gamegrid.gridy / 6) - self.gamegrid.gridy * self.vertical_attackingrange:
                                    attackable = True
                        elif boat.original_stance == "defending":
                            if self.new_x - (self.gamegrid.gridx / 6) + self.gamegrid.gridx * (
                                self.horizontal_attackingrange + 1) > boat.x + self.gamegrid.gridy * tiles > self.new_x - (
                                self.gamegrid.gridx / 6) - self.gamegrid.gridx * self.horizontal_attackingrange:
                                if self.new_y - (
                                    self.gamegrid.gridy / 6) + self.gamegrid.gridy * self.length > boat.y > self.new_y - (
                                    self.gamegrid.gridy / 6):
                                    attackable = True
                            if self.new_x - (
                                self.gamegrid.gridx / 6) + self.gamegrid.gridx > boat.x + self.gamegrid.gridy * tiles > self.new_x - (
                                self.gamegrid.gridx / 6):
                                if self.new_y - (self.gamegrid.gridy / 6) + self.gamegrid.gridy * (
                                    self.length + self.vertical_attackingrange) > boat.y > self.new_y - (
                                    self.gamegrid.gridy / 6) - self.gamegrid.gridy * self.vertical_attackingrange:
                                    attackable = True
                    elif self.new_stance == "defending":
                        if boat.original_stance == "attacking":
                            if self.new_x - (self.gamegrid.gridx / 6) + (
                                self.gamegrid.gridx * self.length) > boat.x > self.new_x - (self.gamegrid.gridx / 6):
                                if self.new_y - (self.gamegrid.gridy / 6) + self.gamegrid.gridy * (
                                    self.vertical_defendingrange + 1) > boat.y + self.gamegrid.gridy * tiles > self.new_y - (
                                    self.gamegrid.gridy / 6) - self.gamegrid.gridy * self.vertical_defendingrange:
                                    attackable = True
                        elif boat.original_stance == "defending":
                            if self.new_x - (
                                self.gamegrid.gridx / 6) > boat.x + self.gamegrid.gridy * tiles > self.new_x - (
                                self.gamegrid.gridx / 6) + (self.gamegrid.gridx * self.length):
                                if self.new_y - (self.gamegrid.gridy / 6) + self.gamegrid.gridy * (
                                    self.vertical_defendingrange + 1) > boat.y > self.new_y - (
                                    self.gamegrid.gridy / 6) - self.gamegrid.gridy * self.vertical_defendingrange:
                                    attackable = True
                    tiles -= 1
                if attackable:
            ...
            if len(Game1.currentplayer.attackable_boats) == 0:
                game_error("Er zijn geen boten in de buurt!", screen)
...
Game1.allcards = [card_adrenaline_rush, card_advanced_rifling, card_aluminium_hull, card_backup, card_emp,
                  card_extra_fuel_2, card_extra_fuel, card_far_sight, card_flak_armor, card_fmj, card_hack_intel,
                  card_jack_sparrow, card_naval_mine, card_rally, card_reinforced_hull, card_repair, card_rifling,
                  card_sabotage, card_smokescreen, card_sonar]
...
def text_to_screen(scherm, text, color, y_displace=0, size="small", x_displace=0):
        ...
        if pygame.mouse.get_pressed()[0] == 1 and action is not None:
            while pygame.mouse.get_pressed()[0] == 1:
                for event in pygame.event.get():
                    if event.type == pygame.MOUSEBUTTONUP and x + width > pygame.mouse.get_pos()[0] > x and y + height > \
...
def gamePause():
    ...
    text_to_screen(screen, "Het spel is gepauzeerd", black, -display_height * 0.15, "medium")
...
def inputName():
    ...
    while not gameExit:
        ...
        screen.fill(white)
        if Game1.currentplayer == P1:
            text_to_screen(screen, "Naam P1: " + str(Game1.currentplayer.name), black, -display_height * 0.35, "medium")
        else:
            text_to_screen(screen, "Naam P2: " + str(Game1.currentplayer.name), black, -display_height * 0.35, "medium")
        if Game1.currentplayer.name:
            if Game1.currentplayer == P1:
                ...
                button("Start game", display_width * 0.825, display_height * 0.81, 190, 60, green, light_blue, black,
                       "chooseboats")
...
def chooseBoats():
    ...
    while not gameExit:
        ...
        if Game1.currentplayer == P1 and len(P1.boatlist) > len(P2.boatlist):
            button("Volgende", display_width * 0.825, display_height * 0.81, 190, 60, green, light_blue, black,
                   "next_player")
            button("Verwijder gekozen boot", display_width * 0.5 - 250, display_height * 0.75, 500, 50, green,
                   light_blue, black, "remove_boat")
        ...
        if len(Game1.available_boats) > 4:
            text_to_screen(screen, (str(Game1)) + ", kies een schip", black, -(display_height * 0.35), "medium")
        else:
            text_to_screen(screen, "Druk op start game", black, -(display_height * 0.35), "medium")
...
def gameIntro():
    ...
    while not gameExit:
        ...
        screen.fill(white)
        text_to_screen(screen, "Battleships", black, -(display_height * 0.35), "medium")
        button("Start game", (display_width / 2) - 75, (display_height * 0.35), 150, 50, red, light_blue, black,
               "inputname")
        button("Help", (display_width / 2) - 75, (display_height * 0.45), 150, 50, red, light_blue, black, "rules_main")
        button("Highscore", (display_width / 2) - 75, (display_height * 0.55), 150, 50, red, light_blue, black,
               "high score")
...
def gameRules(page):
    ...
    while not gameExit:
        ...
        screen.fill(white)
        if page == "main":
            text_to_screen(screen, "Welkom in het regelboek", black, 0)
            text_to_screen(screen, "Navigeer met de knoppen hier rechts", black, +50, "rules")

        elif page == "voorbereiding":
            text_to_screen(screen, "Welkom op pagina voorbereiding", black, -100, "small")
            text_to_screen(screen, "Iedere speler begint met twee schepen", black, -50, "rules")
            text_to_screen(screen, "en 2 kaarten van de basis stapel.", black, -25, "rules")
            text_to_screen(screen, "De spelers plaatsen hun schepen tegen", black, 0, "rules")
            text_to_screen(screen, " zijn/haar eigen haven aan in de aanvalspositie", black, +25, "rules")
            text_to_screen(screen, "de speler mag zelf bepalen waar de schepen staan,", black, +50, "rules")
            text_to_screen(screen, "zolang ze maar tegen zijn/haar eigen haven staat.", black, +75, "rules")

        elif page == "spelverloop":
            text_to_screen(screen, "Welkom op pagina spelverloop", black, -150)
            text_to_screen(screen, "Aan het begin van een speler zijn/haar beurt", black, -100, "rules")
            text_to_screen(screen, "trekt de speler één kaart van de normale stapel.", black, -75, "rules")
            text_to_screen(screen, "Kaarten mogen alleen gebruikt worden voor het aanvallen.", black, -50, "rules")
            text_to_screen(screen, "De speler die aan de beurt is mag zijn schepen verplaatsen,", black, -25, "rules")
            text_to_screen(screen, "wanneer een schip van de tegenstander binnen het aanvals", black, 0, "rules")
            text_to_screen(screen, "bereik staat van de speler die aan de beurt is staat, mag", black, +25, "rules")
            text_to_screen(screen, "de speler die aan de beurt is zijn tegenstanders schip aanvallen.", black, +50, "rules")
            text_to_screen(screen, "Er mag per beurt maximaal 2 keer aangevallen worden", black, +75, "rules")
            text_to_screen(screen, "en elk schip mag maar maximaal 1 keer per beurt aanvallen.", black, +100, "rules")


        elif page == "boten":
            text_to_screen(screen, "Welkom op pagina boten", black, -150)
            text_to_screen(screen, "☻Er zijn verschillende schepen in dit spel", black, -100, "rules")
            text_to_screen(screen, "met elk andere lengtes.", black, -75, "rules")
            text_to_screen(screen, "☻Als een schip vernietigt wordt, zal dit schip op het veld", black, -50, "rules")
            text_to_screen(screen, "blijven liggen als obstakel waar", black, -25, "rules")
            text_to_screen(screen, "niet doorheen gegaan kan worden.", black, 0, "rules")
            text_to_screen(screen, "☻Wanneer een speler al zijn schepen kwijt is,", black, +25, "rules")
            text_to_screen(screen, "verliest deze speler.", black, +50, "rules")
            text_to_screen(screen, "De winnaar is de speler die als laatste", black, +75, "rules")
            text_to_screen(screen, "met een niet vernietigt schip overblijft.", black, +100, "rules")


        elif page == "bewegen & posities":
            text_to_screen(screen, "Welkom op pagina bewegen & posities", black, -225)
            text_to_screen(screen, "☻Wanneer een speler aan de beurt is mag de speler", black, -175, "rules")
            text_to_screen(screen, "al zijn/haar schepen verplaatsen volgens het aantal", black, -150, "rules")
            text_to_screen(screen, "stappen dat het schip kan zetten.", black, -125, "rules")
            text_to_screen(screen, "☻Ook kun je de positie van je schepen veranderen,", black, -100, "rules")
            text_to_screen(screen, "wanneer je dit doet telt dat als 1 stap:", black, -75, "rules")
            text_to_screen(screen, "    ☺Wanneer een schip in zijn aanvalspositie staat heeft", black, -50, "rules")
            text_to_screen(screen, "    het schip zijn standaard aanval bereik. ", black, -25, "rules")
            text_to_screen(screen, "    ☺Wanneer een schip in zijn verdedigingspositite staat", black, 0, "rules")
            text_to_screen(screen, "    mag deze niet verplaatst worden.", black, +25, "rules")
            text_to_screen(screen, "    (Hulpkaarten hebben nog wel effect)", black, +50, "rules")
            text_to_screen(screen, "☻Spelers mogen 2 keer per beurt aanvallen.", black, +75, "rules")
            text_to_screen(screen, "Aanvallen kan alleen wanneer een schip van de", black, +100, "rules")
            text_to_screen(screen, "tegenstander in het bereik.", black, +125, "rules")
            text_to_screen(screen, "staat van een van jouw schepen.", black, +150, "rules")
            text_to_screen(screen, "Per schip mag je maar één keer aanvallen.", black, +175, "rules")


        elif page == "kaarten":
            text_to_screen(screen, "Welkom op pagina kaarten", black)
            text_to_screen(screen, "Pagina nog niet gevonden.", black, +50, "rules")

        if not page == "voorbereiding":
            button("Voorbereiding", display_width * 0.75, display_height * 0.2, 250, 60, red, light_blue, black,
                   "rules_voorbereiding")
        if not page == "spelverloop":
            button("Spelverloop", display_width * 0.75, display_height * 0.3, 250, 60, red, light_blue, black,
                   "rules_spelverloop")
        if not page == "boten":
            button("Boten", display_width * 0.75, display_height * 0.4, 250, 60, red, light_blue, black, "rules_boten")
        if not page == "bewegen & posities":
            button("Bewegen & posities", display_width * 0.75, display_height * 0.5, 250, 60, red, light_blue, black,
                   "rules_bewegen&posities")
        if not page == "kaarten":
            button("Kaarten", display_width * 0.75, display_height * 0.6, 250, 60, red, light_blue, black,
                   "rules_kaarten")
...
def gameLoop():
    ...
    while not gameExit:
            ...
            if event.type == pygame.QUIT:
                ...
                if not attacking:
                    ...
                    if not setup:
                        if event.key == pygame.K_SPACE:
                            ...
                            if Game1.currentplayer.currentboat.attack_amount > 0:
                                Game1.currentplayer.currentboat.attack_check(enemy)
                                if len(Game1.currentplayer.attackable_boats) > 0:
                                    attacking = True
                                    Game1.currentplayer.targeted_boat = Game1.currentplayer.attackable_boats[0]
                elif attacking:
                    if event.key == pygame.K_SPACE:
                        ...
                        Game1.currentplayer.attack(Game1.currentplayer.targeted_boat, enemy)
                        attacking = False
                    elif event.key == pygame.K_BACKSPACE:
                        attacking = False
        ...
        if not setup:
            ...
            Game1.currentplayer.show_enemy_stats(screen, enemy)
        ...
        if attacking:
            Game1.currentplayer.show_target_stats(screen, enemy)
